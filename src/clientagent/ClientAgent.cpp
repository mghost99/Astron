...
static ConfigVariable<string> bind_addr("bind", "0.0.0.0:7198",
                                        clientagent_config);
static ConfigVariable<string> server_version("version", "dev",
                                             clientagent_config);
static ConfigVariable<bool> behind_haproxy("haproxy", false,
                                           clientagent_config);
...
static ConfigVariable<channel_t> min_channel("min", INVALID_CHANNEL,
                                             channels_config);
static ConfigVariable<channel_t> max_channel("max", INVALID_CHANNEL,
                                             channels_config);
...
KeyedConfigGroup ca_client_config("client", "type", clientagent_config,
                                  "libastron");
...
ConfigConstraint<string>
    client_type_exists(have_client_type, ca_client_type,
                       "No Client handler exists for the given client type.");

static ConfigGroup tuning_config("tuning", clientagent_config);
static ConfigVariable<unsigned long> interest_timeout("interest_timeout", 500,
                                                      tuning_config);

ClientAgent::ClientAgent(RoleConfig roleconfig)
    : Role(roleconfig), m_net_acceptor(nullptr),
      m_server_version(server_version.get_rval(roleconfig)) {

  stringstream ss;
  ss << "Client Agent (" << bind_addr.get_rval(roleconfig) << ")";
  m_log =
      std::unique_ptr<LogCategory>(new LogCategory("clientagent", ss.str()));

  // We need to get the client type...
  ConfigNode client =
      clientagent_config.get_child_node(ca_client_config, roleconfig);
  m_client_type = ca_client_type.get_rval(client);

  // ... and also the channel range ...
  ConfigNode channels =
      clientagent_config.get_child_node(channels_config, roleconfig);
  m_ct = ChannelTracker(min_channel.get_rval(channels),
                        max_channel.get_rval(channels));
  ...
  ConfigNode tuning =
      clientagent_config.get_child_node(tuning_config, roleconfig);
  m_interest_timeout = interest_timeout.get_rval(tuning);

  TcpAcceptorCallback callback = std::bind(
      &ClientAgent::handle_tcp, this, std::placeholders::_1,
      std::placeholders::_2, std::placeholders::_3, std::placeholders::_4);
  AcceptorErrorCallback err_callback =
      std::bind(&ClientAgent::handle_error, this, std::placeholders::_1);

  m_net_acceptor =
      std::unique_ptr<TcpAcceptor>(new TcpAcceptor(callback, err_callback));
...
void ClientAgent::handle_tcp(const std::shared_ptr<uvw::TcpHandle> &socket,
  ...
  m_log->debug() << "Got an incoming connection from " << remote.ip << ":"
                 << remote.port << "\n";

  ClientFactory::singleton().instantiate_client(
      m_client_type, m_clientconfig, this, socket, remote, local, haproxy_mode);
}
 ...
 *       HELPER CLASSES       *
 * ========================== */
ChannelTracker::ChannelTracker(channel_t min, channel_t max)
    : m_next(min), m_max(max), m_unused_channels() {}
