...
static void bamboo2bson(single_context builder,
                        const dclass::DistributedType *type,
  ...
  case dclass::Type::T_BLOB: {
    ...
    if (blob.data() == nullptr) {
      ...
      builder << b_binary{bsoncxx::binary_sub_type::k_binary, 0,
                          (const uint8_t *)1};
    } else {
      builder << b_binary{bsoncxx::binary_sub_type::k_binary,
                          static_cast<uint32_t>(blob.size()), blob.data()};
    }
  } break;
...
template <typename T>
T handle_bson_number(const bsoncxx::types::bson_value::view &value) {
  int64_t i;
  double d;
  ...
  try {
    ...
    case dclass::Type::T_CHAR: {
      if (value.type() != bsoncxx::type::k_string ||
          value.get_string().value.size() != 1) {
        throw ConversionException(
            "Expected single-length string for char field");
      }
    ...
    case dclass::Type::T_ARRAY: {
      ...
      const dclass::DistributedType *element_type =
          type->as_array()->get_element_type();
    ...
    case dclass::Type::T_STRUCT: {
      ...
      for (unsigned int i = 0; i < fields; ++i) {
        ...
        if (!element) {
          throw ConversionException("Struct field is absent",
                                    field->get_name());
        }
        bson2bamboo(field->get_type(), field->get_name(), element.get_value(),
                    dg);
      }
    } break;
    case dclass::Type::T_METHOD: {
      ...
      for (unsigned int i = 0; i < parameters; ++i) {
        ...
        if (!element) {
          throw ConversionException("Parameter is absent",
                                    parameter->get_name());
        }
...
public:
  MongoDatabase(ConfigNode dbeconfig, doid_t min_id, doid_t max_id)
      : DatabaseBackend(dbeconfig, min_id, max_id), m_shutdown(false) {
    ...
    if (!globals) {
      client[m_uri.database()]["astron.globals"].insert_one(
          document{} << "_id" << "GLOBALS"
                     << "doid" << open_document << "monotonic"
                     << static_cast<int64_t>(min_id) << "free" << open_array
...
private:
  ...
  mongocxx::client new_connection() { return (mongocxx::client{m_uri}); }
  ...
  void handle_create(mongocxx::database &db, DBOperation *operation) {
    ...
    try {
      ...
      m_log->error() << "While formatting " << operation->dclass()->get_name()
    ...
    try {
      db["astron.objects"].insert_one(
          document{} << "_id" << static_cast<int64_t>(doid) << "dclass"
                     << operation->dclass()->get_name() << "fields" << fields
                     << finalize);
    } catch (const mongocxx::operation_exception &e) {
      m_log->error() << "Cannot insert new " << operation->dclass()->get_name()
                     << "(" << doid << "): " << e.what() << endl;
      operation->on_failure();
      return;
    }
  ...
  void handle_delete(mongocxx::database &db, DBOperation *operation) {
    bool success;
    try {
      auto result = db["astron.objects"].delete_one(
          document{} << "_id" << static_cast<int64_t>(operation->doid())
                     << finalize);
      success = result && (result->deleted_count() == 1);
    } catch (const mongocxx::operation_exception &e) {
      m_log->error() << "Unexpected error while deleting " << operation->doid()
                     << ": " << e.what() << endl;
      operation->on_failure();
      return;
    }
  ...
  void handle_get(mongocxx::database &db, DBOperation *operation) {
    ...
    try {
      obj = db["astron.objects"].find_one(
          document{} << "_id" << static_cast<int64_t>(operation->doid())
                     << finalize);
    } catch (const mongocxx::operation_exception &e) {
  ...
  void handle_modify(mongocxx::database &db, DBOperation *operation) {
    ...
    if (!sets.view().empty())
      updates_builder << "$set" << sets;
    if (!unsets.view().empty())
      updates_builder << "$unset" << unsets;
    ...
    for (const auto &it : operation->criteria_fields()) {
      ...
      fieldname << "fields." << it.first->get_name();
      if (it.second.empty()) {
        query << fieldname.str() << open_document << "$exists" << false
              << close_document;
      } else {
        DatagramPtr dg = Datagram::create();
        dg->add_data(it.second);
    ...
    bsoncxx::stdx::optional<bsoncxx::document::value> obj;
    try {
      obj = db["astron.objects"].find_one_and_update(query_obj.view(),
                                                     updates.view());
    } catch (const mongocxx::operation_exception &e) {
      m_log->error() << "Unexpected error while modifying " << operation->doid()
                     << ": " << e.what() << endl;
      operation->on_failure();
      return;
    }
    ...
    if (obj) {
      ...
      m_log->trace()
          << "Update result: [server could not find a matching object]" << endl;
    }
    ...
    auto obj_v = obj->view();
    string dclass_name = string(obj_v["dclass"].get_string().value);
    const dclass::Class *dclass = g_dcf->get_class_by_name(dclass_name);
    if (!dclass) {
      m_log->error() << "Encountered unknown database object: " << dclass_name
                     << "(" << operation->doid() << ")" << endl;
  ...
  // Get a DBObjectSnapshot from a MongoDB BSON object; returns nullptr if
  // failure.
  DBObjectSnapshot *format_snapshot(doid_t doid, bsoncxx::document::view obj) {
    ...
    string dclass_name = string(obj["dclass"].get_string().value);
    const dclass::Class *dclass = g_dcf->get_class_by_name(dclass_name);
    if (!dclass) {
      m_log->error() << "Encountered unknown database object: " << dclass_name
                     << "(" << doid << ")" << endl;
    ...
    try {
      ...
      m_log->error() << "Unexpected error while trying to format"
                        " database snapshot for "
                     << dclass_name << "(" << doid << "): " << e.what() << endl;
      delete snap;
      return nullptr;
    }
  ...
  doid_t assign_doid_monotonic(mongocxx::database &db) {
    auto obj = db["astron.globals"].find_one_and_update(
        document{} << "_id" << "GLOBALS"
                   << "doid.monotonic" << open_document << "$gte"
                   << static_cast<int64_t>(m_min_id) << close_document
                   << "doid.monotonic" << open_document << "$lte"
                   << static_cast<int64_t>(m_max_id) << close_document
                   << finalize,
        document{} << "$inc" << open_document << "doid.monotonic" << 1
    ...
    doid_t doid = handle_bson_number<doid_t>(
        obj->view()["doid"]["monotonic"].get_value());
    return doid;
  }
  ...
  void free_doid(mongocxx::database &db, doid_t doid) {
    m_log->trace() << "Returning doid " << doid << " to the free pool..."
                   << endl;
    ...
    try {
      db["astron.globals"].update_one(
          ...
          document{} << "$push" << open_document << "doid.free"
                     << static_cast<int64_t>(doid) << close_document
                     << finalize);
    } catch (const mongocxx::operation_exception &e) {
