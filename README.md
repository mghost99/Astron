...
```
sudo apt install bison curl flex g++ git gnupg libboost-dev libicu-dev libsasl2-dev libssl-dev libuv1-dev libyaml-cpp-dev libzstd-dev pkg-config 
```
...
echo "deb https://libmongocrypt.s3.amazonaws.com/apt/ubuntu focal/libmongocrypt/1.8 universe" | sudo tee /etc/apt/sources.list.d/libmongocrypt.list
sudo apt-get update
