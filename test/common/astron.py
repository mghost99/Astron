    ...
    def start(self):
        if 'MANUAL_LAUNCH_CONFIG' in os.environ:
            ...
            with open(os.environ['MANUAL_LAUNCH_CONFIG'], 'wb') as config:
                config_data = self.config if isinstance(self.config, bytes) else self.config.encode('utf-8')
                config.write(config_data)

            input('Waiting for manual launch; press enter when ready...')

            return # Because the start happened manually.

        configHandle, self.config_file = tempfile.mkstemp(prefix = 'astron', suffix = 'cfg.yaml')
        config_data = self.config if isinstance(self.config, bytes) else self.config.encode('utf-8')
        os.write(configHandle, config_data)
...
class Datagram(object):
    ...
    def get_payload(self):
        num_channels = self._data[0] if isinstance(self._data[0], int) else ord(self._data[0])
        return self._data[CHANNEL_SIZE_BYTES*num_channels+1:]
...
class DatagramIterator(object):
    ...
    def matches_header(self, recipients, sender, msgtype, remaining=-1):
        ...
        num_channels = self._data[0] if isinstance(self._data[0], int) else ord(self._data[0])
        self.seek(CHANNEL_SIZE_BYTES*num_channels+1)
